...
class ManageSetmenu extends React.Component {
  ...
  handleChangePrice = (setmenuItem, e) => {
    ...
    this.forceUpdate(); // 가격 수정 시도 시 숫자가 제대로 변하지 않는 문제가 있어서 이 코드를 추가함.
  };

  handleOnBlur = (setmenuItem) => {
    const url = this.props.api_url + "/v1/setmenu/" + (setmenuItem.id).toString();
    const headers = {
      "Authorization": "Bearer " + this.props.jwt
    };
  ...
  handleNewSetmenuSubmit = (e) => {
    ...
    const url = this.props.api_url + "/v1/setmenu";
    const headers = {
      "Authorization": "Bearer " + this.props.jwt
    };
      ...
      <React.Fragment>
        ...
        <Paper className={classes.root}>
          ...
          <FormControl fullWidth className={classes.formField}>
            ...
            <Button fullWidth color="primary" variant="contained" onClick={this.handleNewSetmenuSubmit}>추가하기</Button>
          </FormControl>
        </Paper>
      </React.Fragment>
    );
