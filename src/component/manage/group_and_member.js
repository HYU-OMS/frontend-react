...
import { Paper, Typography, Hidden, TextField, FormControl, Button, Divider } from 'material-ui';
...
class ManageGroupAndMember extends React.Component {
  constructor(props) {
    super(props);
  ...
  handleOnSubmit = (e) => {
    ...
    const url = this.props.api_url + "/api/group/" + (this.props.group_id).toString() + "?jwt=" + this.props.jwt;
  ...
  deleteMember = (member) => {
    if(window.confirm("이 멤버를 제거하시겠습니까?")) {
      const url = this.props.api_url + "/api/member?jwt=" + this.props.jwt +
        "&group_id=" + (this.props.group_id).toString() + "&user_id=" + (member.id).toString();
  ...
  downloadOrderList = () => {
    const url = this.props.api_url + "/api/download?jwt=" + this.props.jwt +
      ...
      <React.Fragment>
        ...
        <Paper className={classes.root}>
          ...
          <FormControl fullWidth className={classes.formField}>
            ...
            <Button fullWidth color="primary" variant="raised" onClick={this.handleOnSubmit}>변경하기</Button>
          </FormControl>
        </Paper>
...
export default connect(
  mapStateToProps,
  mapDispatchToProps
)(withStyles(styles)(ManageGroupAndMember));